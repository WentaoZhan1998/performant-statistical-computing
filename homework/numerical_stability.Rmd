...
svd <- svd(Sigma_obs)
k = which.max(cumsum(svd$d)>0.9999*sum(svd$d))
...
points(x = loc_new, y = CI_U, type = 'l')
points(x = loc_new, y = CI_L, type = 'l')
```
...
k_m = which.max(cumsum(svd_m$d)>0.9999*sum(svd_m$d))
Sigma_k_matern <- svd_m$u[,1:k_m] %*% diag(1/svd_m$d[1:k_m]) %*% t(svd_m$v[,1:k_m])
