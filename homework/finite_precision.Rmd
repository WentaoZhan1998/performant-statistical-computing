...
count_duplicate <- function(n_draw){
  sample <- runif(n_draw)

  return(sum(duplicated(sample+1-1))) # +1-1 to avoid the problem caused by the accuracy near 0.
}

est_duplicate <- mean(replicate(1000, count_duplicate(n_draw)))
expected_duplicate <- est_num_expected_duplicate(n_draw, n_unique_double)

print(sprintf(
  "The expected number of duplicates is %.3g while the estimated number of duplicates is %.3g. (The estimator comes from the mean of 1000 trials)", 
  expected_duplicate, est_duplicate
))
```
\emph{The expected number does not agree with the estimated one and there seems to be a $10^6$ times difference. A guess on the phenomenon is that the random number generating process ('runif') does not reach the accuracy of $2^{-52}$.}
...
\emph{A slope of $-2$ is expected since the error is of rate $O(\Delta x^2)$.}
...
\emph{The plotted line has two parts. The first part has slope $-2$, which is the same as expected. However, the second part has slope approximately $1$. The phenomenon comes from the formula $f(x+\Delta x)-f(x-\Delta x)$, which should be expressed by $2xf'(x)+C(\Delta x^3)+o(\Delta x^3) + \delta$. The last term is the system error of the order $10^{-16}$. When $\Delta x$ decreases, the error $C(\Delta x^3)$ will be dominated by $\delta$ after the threshold $2^{-52/3}$. And the dominant error then will be $\delta/2\Delta x = O(1/\Delta x)$, the slope of the line is thus approximately $1$ instead of $-2$.}

**Bonus question 1:** What is the slope in the "latter part" of your plotted line?

\emph{Explained in the last part.}
