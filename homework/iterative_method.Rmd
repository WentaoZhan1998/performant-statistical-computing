...
required_packages <- c('RSpectra', 'CVTuningCov')
...
ar_precision_matvec <- function(v, auto_corr) {
  d = rep(1,ar_length)
  d[2:(ar_length-1)] = (1+auto_corr^2)*d[2:(ar_length-1)]
  
  m = diag(d)
  m[abs(row(m) - col(m)) == 1] = -auto_corr
...
# The running time of the function seems pretty long and can't get a result. Don't know why.
```
...
decomp_1 <- eigs_sym(Sigma, k=10)
decomp_2 <- eigen(Sigma)

result = data.frame('Lancsoz' = decomp_1$values[1:10], 'Eigen_decomp' = decomp_2$values[1:10])
knitr::kable(result)
```
The two method give same result for the first 10 eigenvalues.
